...
export const ExportableSingleGame = forwardRef<HTMLDivElement, SingleGameExportProps>(({ game, user, year, listName, listType = 'game' }, ref) => {
    ...
    const renderContent = () => (
        <>
            ...
            {/* Review/Reason */}
            {game.reason && (
                <Box bg="rgba(255,255,255,0.05)" p="sm" mt="md" style={{ borderRadius: 8, height: listType === 'manga' ? 160 : 120 }}>
                    <Group gap="xs" mb={4}>
                        <IconQuote size={16} color="gray" style={{ opacity: 0.5 }} />
                        <Text size="xs" fw={700} c="dimmed">评价</Text>
                    </Group>
                    <Text size="sm" c="white" style={{ whiteSpace: 'pre-wrap', lineHeight: 1.5 }} lineClamp={listType === 'manga' ? 5 : 3}>
                        {game.reason}
                    </Text>
                </Box>
            )}
